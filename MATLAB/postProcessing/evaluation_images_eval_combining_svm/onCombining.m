...
function [ DiceCof , Accuracy ] = onCombining(truth , im , classifier_name, layer)
	%{
		Inputs:
			im 				: input image
			truth			: ground truth image
			classifier_name	: file name of svm classifier
			layer			: layer name to extract features
			...
			overlapping patches(128 x 128) are taken with stride of [ 20 20]. model is evaluated using the patches.
				...
				.
				.
			Then patch to Whole Image Classification is evaluated using MAX, SUM and MOJORITY VOTE patch combining methods
			are evaluated. 
			Finally classification metrics dice coeficient and accuracy are evaluated.
	...
	for i=patch_row_half:stride(1):X
		...
		for j=patch_col_half:stride(2):Y
			...
			if tissue_percentage< tissue_percentage_thresh
				...
				% run the CNN
				%net.eval({'input', patch});

				activation = getLayerActivation(net, patch , layer);
				activation  = transpose (squeeze(gather(activation)));				

				[label,~,~,classiff_output] = predict(classifier,activation);
				%classiff_output = cell2mat(classiff_output);
